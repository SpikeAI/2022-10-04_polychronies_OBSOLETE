...
\begin{figure}%[t!]
  ...
  \includegraphics[width=0.980\linewidth]{figures/izhikevitch_repro.png}
    \caption{\textbf{Core mechanism of polychrony detection~\citep{izhikevich_polychronization_2006}.} \textit{(Left)}~In that example, three presynaptic neurons denoted \textit{b}, \textit{c} and, \textit{d} are fully connected to two post-synaptic \textit{a} and \textit{e}, with different delays of respectively $1$, $5$, and $9~\ms$ for \textit{a} and  $8$, $5$, and $1~\ms$ for \textit{e}. \textit{(Middle)}~if a synchronous pulse is emitted in all presynaptic neurons, this will generate post-synaptic potentials at these different delays, and these may not be sufficient to generate a spike in either neuron. \textit{(Right)}~If a sequence of activation is generated in the presynaptic neurons such that, taking into account the heterogeneous delays, these converge at the same time on neuron \textit{a} (here, at $t=10~\ms$) then a spike is emitted (here in red color), while none is emitted on neuron \textit{e}.
     }
