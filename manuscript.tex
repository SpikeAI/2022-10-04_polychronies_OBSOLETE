...
\begin{figure}
\centering
\includegraphics[width=.61803\textwidth]{figures/visual-latency.pdf}
% thorpe
% \includegraphics[width=.7\textwidth]{figures/visual-latency-estimate.jpg}%-estimate.jpg}
\caption{{\bf Latency of the different processing steps along the visual pathway.} Though the visual system is a highly inter-connected and complex system, one can follow the sequence of activations whenever an image (here a yellow star) is flashed in front of the eyes. Different areas are schematically represented by ellipses and arrows denotes the fastest feed-forward activation, ordered with respect to their activation latency. In order, the retina is first activated ($20-40~\ms$) then the thalamus and the primary visual cortex (V1, $60-80~\ms$). Visual information used for object recognition follows the temporal lobe to reach the infero-temporal area (IT, $100-120~\ms$), and then the prefrontal cortex (PFC) which modulates decision making and the Motor cortex (MC, $140-190~\ms$) which mediates a motor action which is then relayed through the spinal chord to to finger muscles, with latencies about $200-350~\ms$. Figured inspired by a similar schematics done for monkeys in~\citep{thorpe_seeking_2001}. The latencies in macaque monkey are about: Retina, $20-40~\ms$; V1, $40-60~\ms$; IT, $80-100~\ms$; MC, $140-190~\ms$; to finger mescles, $180-260~\ms$.}\label{fig:thorpe}
\end{figure}
%----------------------------%
